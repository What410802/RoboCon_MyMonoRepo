...
- 现有可瘦身项（**待确认后再动**，见 2026-09-26 的梳理）：任务 README 的「URDF → MJCF 两条路线」「网站导出选项」「任务 2 结果」「录像工具：怎么接入」四块与 `docs/learn/mujoco.md`、`docs/pitfalls/environment.md` 有重叠，适合拆到 `@20260923_mujoco/docs/` 下，仓库 `docs/` 只留通用规则 + 指针。
