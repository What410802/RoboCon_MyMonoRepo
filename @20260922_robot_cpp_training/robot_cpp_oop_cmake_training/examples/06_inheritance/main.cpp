...
public:
    ...
    {
        position_ = position;
        std::cout << "DM Motor " << id_ << " -> target = " << std::fixed << std::setprecision(2) << position_ << " rad" << std::endl;
    }
...
public:
    ...
    {
        position_ = position;
        std::cout << "Unitree Motor " << id_ << " -> target = " << std::fixed << std::setprecision(2) << position_ << " rad" << std::endl;
    }
    double getPosition() const override { return position_; }
private:
    int id_;
...
int main()
{
    ...
    dm_motor.setPosition(1.5);
    unitree_motor.setPosition(1.5);
    return 0;
}
